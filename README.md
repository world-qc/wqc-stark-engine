...
- **SNARK wrap of $\pi_{\text{Root}}$:** thin toolchain done in **[`wqc-snark-wrap`](https://github.com/world-qc/wqc-snark-wrap)** (E5b-2a–2c; E5b-2d thin KPIs). **E5b-3a/3b/3c Done**; **E5b-3d Partial** (`thick_fri_fs_auth_v0` N=40 + `thick_recagg_fri_fs_auth_v0` N=40 + OOD/AIR↔Auth binds incl. Quotient↔recompose(QuotOpen) + `thick_child_verify_d3_v0` ~3.041M + `thick_leaf_fri_fs_auth_v0` N=1 Trace+Quot+FL+DeepRo→FL ~1.754M folded into `thick_unified_v0` ~387.10M est); wrap gate still needs multi-chunk Quot zps(ζ), variable compose, full leaf Auth, Settlement VK — **thin_v0 keys remain** (`on-chain_settlement_scope.md` §7.2 / §7.6). Wrap goldens in `fixtures/e5b/wrap_*_golden.json`. This repo stays the STARK engine / authoritative off-chain verify.
