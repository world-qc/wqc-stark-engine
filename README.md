...
- **SNARK wrap of $\pi_{\text{Root}}$:** thin toolchain done in **[`wqc-snark-wrap`](https://github.com/world-qc/wqc-snark-wrap)** (E5b-2a–2c; E5b-2d thin KPIs). **E5b-3a/3b/3c Done**; **E5b-3d Partial** (`thick_fri_fs_auth_v0` N=40 + `thick_recagg_fri_fs_auth_v0` N=40 + OOD/AIR↔Auth binds incl. Quotient↔recompose(QuotOpen) + `thick_child_verify_var_v0` Depth∈{1..4}+soft Auth×2 ~19.711M + `thick_leaf_fri_fs_auth_v0` N=1 FriFsChal+Auth ~4.100M + OOD↔leaf Trace/α/Quotient multi-chunk zps(ζ) in `thick_unified_v0` ~405.96M est + standalone `thick_child_verify_leaf_auth_v0` Depth=1 Auth×2 ~8.431M + `thick_child_digest_var_v0` Len∈{1..256} ~45.878M); wrap gate still needs Max→87KiB / deeper Auth slots / payload↔Auth, Settlement VK — **thin_v0 keys remain**; thick Settlement VK **working target ≤20M R1CS** (N-ladder soft-est `fixtures/e5b/wrap_r1cs_n_ladder_soft_est.json`; production N=40 unchanged; Phase-2: Audit dropped from unified, soft Auth no elision, digest_var fold forbidden) (`on-chain_settlement_scope.md` §7.2 / §7.6). Wrap goldens in `fixtures/e5b/wrap_*_golden.json`. This repo stays the STARK engine / authoritative off-chain verify.
