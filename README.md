...
- **SNARK wrap of $\pi_{\text{Root}}$:** thin toolchain done in **[`wqc-snark-wrap`](https://github.com/world-qc/wqc-snark-wrap)** (E5b-2a–2c; E5b-2d thin KPIs). **E5b-3a/3b/3c Done**; **E5b-3d Partial** (`thick_fri_varlog_v0` + `thick_fri_pack_v0` + `thick_unified_v0` folds all shipped gadgets incl. FriVarLog + FriPack + depth-2 child-verify + unitary/Born/Traj leaf binds); wrap gate still needs FS bind / N=40 / full PCS / depth≥3 compose / full leaf verify / VK (`on-chain_settlement_scope.md` §7.2 / §7.6). Wrap goldens in `fixtures/e5b/wrap_*_golden.json`. This repo stays the STARK engine / authoritative off-chain verify.
