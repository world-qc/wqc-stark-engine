...
**Wrap status (sibling repos):** thin Groth16 toolchain (E5b-2a–2c) and thin KPI locks (E5b-2d partial) live in [`wqc-snark-wrap`](https://github.com/world-qc/wqc-snark-wrap) + `wqc-contracts` (§7.5). **E5b-3a/3b/3c Done**; **E5b-3d Partial** — RecAgg AIR + Unitary/Shot leaf OOD. Full ≡ `verify_root_proof` / VK cutover still open. Until then, keep `verify_root_proof` / FFI as the authoritative root check (E5a challenge path).
