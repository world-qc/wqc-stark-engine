...
fn push_commitment_or_zero(
    ...
    match (kind, cert) {
        ...
        (REC_KIND_AGG, Some(cert)) => {
            push_bytes(row, &cert.trace_commitment);
            Ok(())
        }
        (REC_KIND_AGG, None) => Err("kind=agg requires AggregationAir PCS certificate".into()),
        (REC_KIND_LEAF, Some(_)) => {
            Err("kind=leaf must not carry AggregationAir certificate".into())
        }
        (k, _) => Err(format!("invalid kind {k}")),
    }
}
...
fn build_rec_matrix(
    ...
    push_commitment_or_zero(&mut row, ctx.left_kind, &ctx.left_agg_cert)?;
    debug_assert_eq!(row.len(), REC_RIGHT_TRACE_COM_COL);
    push_commitment_or_zero(&mut row, ctx.right_kind, &ctx.right_agg_cert)?;
    debug_assert_eq!(row.len(), REC_LEFT_AGG_ROW_COL);
    push_natural_row_or_zero(&mut row, ctx.left_kind, &ctx.left_agg_cert)?;
    debug_assert_eq!(row.len(), REC_RIGHT_AGG_ROW_COL);
    push_natural_row_or_zero(&mut row, ctx.right_kind, &ctx.right_agg_cert)?;
...
pub fn verify_recursive_aggregation_proof(
    ...
    let (plonky3_bytes, use_m1_air) =
        if let Some(bytes) = decode_rec_agg_proof_owned_v6(proof, context) {
            ...
            (bytes, false)
        } else if context.left_agg_cert.is_none() && context.right_agg_cert.is_none() {
